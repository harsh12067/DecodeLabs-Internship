...
function CountUp({ end, duration = 1500, suffix = "" }) {
  ...
  return (
    <span ref={containerRef} className="text-4xl sm:text-5xl font-extrabold bg-gradient-to-r from-purple-500 via-indigo-500 to-pink-500 bg-clip-text text-transparent">
      {count}{suffix}
    </span>
  );
}
...
export default function Achievements() {
  const achievements = [
    ...
    },
    {
      ...
      value: 8,
      suffix: '+',
      icon: <FaCertificate className="w-6 h-6 text-pink-500" />
    },
    {
      id: 4,
      ...
      value: 500,
      suffix: '+',
      icon: <FaHourglassHalf className="w-6 h-6 text-blue-500" />
    }
  ];
  ...
  return (
    <section id="achievements" className="py-20 relative overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[300px] rounded-full bg-purple-500/5 blur-[120px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          {achievements.map((item, index) => (
            <div 
              key={item.id} 
              className="glass-card p-6 sm:p-8 rounded-3xl text-center flex flex-col items-center justify-center space-y-4 hover:scale-105 transition-all duration-300 shadow-md"
              data-aos="zoom-in"
              data-aos-delay={index * 100}
            >
              {/* Icon */}
              <div className="p-4 bg-slate-100 dark:bg-navy-800 rounded-2xl">
                {item.icon}
              </div>
